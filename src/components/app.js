...
export default class App extends Component {
    constructor(props) {
        ...
        this.state = {
            currentState: {value: '', label: ''},
            currentDistrict: {value: '', label: ''}
        };
  ...
  assignState(newState) {
      this.setState({currentState: newState, currentDistrict: {value: '', label: ''}});
  }
  ...
  render() {
    return (
      ...
      <nav className="navbar navbar-dark bg-dark">
        <h1 className="big-name"><span className="highlight">Margin of Victory Visualizations: </span><em>US House of Representatives (2016)</em></h1>
        <form className="form-inline my-2 mt-4 my-lg-0">
          <label className="mr-sm-2">State:</label>
        <StateSelector
          className="mr-sm-2"
                currentState={this.state.currentState}
                onStateChange={this.assignState}/>
              <label className="mr-sm-2">District:</label>
        <DistrictSelector
          className="mr-sm-2"
                currentState={this.state.currentState}
                currentDistrict={this.state.currentDistrict}
                onDistrictChange={this.assignDistrict} />
              </form>
      </nav>
